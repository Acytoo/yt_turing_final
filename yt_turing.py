...
class TuringMachine:
    ...
    def compute(self, input_tape,
        ...
        if print_results:
            ...
            text = Text(top, font = (None, 13), width = 200)
            ...
            for step, state, tape, transition in zip(range(num_iter + 1),
                                                     ...
                                                     all_tapes,
                                                     all_transitions):
                yt_str = (('步数= {i:' + str(w_step) + '} \t' +
                            '状态= {s:' + str(w_state) + '} \t' +
                            '纸带= {t:' + str(w_tape) + '} \t' +
                            '状态转移方式= {n}').format(i=step, s=state, t=tape,
                                                 n=transition))
                text.insert(END, yt_str)
                text.insert(END, '\n')
            b1 = Button(text,text='确定',command=lambda m = top :exit_chile(m))
            text.window_create(INSERT,window=b1)
            text.pack()
        return current_tape, yt_str
